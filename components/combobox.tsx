...
const Combobox = forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    // Group options upfront to simplify grouped rendering.
    ...
    // Debounce user input for async search.
    ...
    // If `onSearch` is provided, options are treated as remote and not synced here.
    useEffect(() => {
        ...
        <div className="flex min-h-fit flex-col gap-1">
          ...
          >
            <PopoverTrigger className="relative">
              <Input
                className={input({ className: className as string })}
                disabled={disabled}
                value={inputValue}
                size={size}
                variant={variant}
            ...
            {/* Hidden cmdk input keeps internal filtering in sync with our visible Input. */}
