...
function Tree({
  ...
  // Support controlled/uncontrolled selection state.
  ...
  return (
    ...
    >
      <ul
        data-slot="tree"
        className={root({ size, appearance, className })}
        {...props}
      >
        {children}
      </ul>
...
function TreeItem({
  ...
  // First child is the layout, remaining children are nested tree items.
  const allChildren = React.Children.toArray(children)
  const layoutChild = allChildren[0]
  const nestedChildren = allChildren.slice(1)

  return (
    <li data-slot="tree-item" className={item({ className: className as string })} {...props}>
      {React.isValidElement(layoutChild)
        ? React.cloneElement(layoutChild as React.ReactElement<any>, {
      ...
      {isBranch && isOpen && nestedChildren.length > 0 && (
        <ul className="ml-4 flex flex-col gap-1">{nestedChildren}</ul>
      )}
    </li>
  )
}
  ...
  return (
    <div
      ...
      className={row({
        ...
        selected: context?.selectedItems.includes(
          String((props as { "data-tree-item"?: string })["data-tree-item"] ?? "")
        ),
