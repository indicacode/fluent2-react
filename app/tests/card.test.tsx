...
describe("Card Component", () => {
  ...
  it("should apply the correct size classes", () => {
    ...
    expect(card.parentElement).toHaveClass(
      " relative flex h-fit w-full flex-col overflow-hidden rounded-md border shadow-md"
    ) // `lg` size class
  })
  ...
  it("should apply the correct variant classes", () => {
    ...
    expect(card.parentElement).toHaveClass(
      "border border-border shadow-none text-foreground hover:bg-muted"
    )
  })
  ...
  it("should toggle `checked` state when selectable", () => {
    ...
    expect(card).not.toHaveClass("border-border") // Not checked initially
    fireEvent.click(card!)
    expect(card).toHaveClass("border-border") // Checked after click
  })
...
describe("CardHeader Component", () => {
  it("should render the CardHeader with children", () => {
    render(
      <CardHeader>
        <h2 className={"flex w-fit items-center gap-3"}>Header Content</h2>
      </CardHeader>
    )
